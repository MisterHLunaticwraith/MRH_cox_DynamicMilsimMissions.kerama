...
//_displaycreated = with uiNamespace do {findDisplay 46 ctrlCreate ["RscPicture", -1]};
_layer = if (count _this > 6) then {_this select 6} else {["RscPicture"] call bis_fnc_rscLayer}; //edit
...
_textheight = ctrlTextHeight _control; //editsperso
...
if (_moveY != 0) then {
	...
	_y = _pos select 1;
	_pos set [1,_y + _moveY];////editsperso 4èmeparam
	_control ctrlsetposition _pos;
